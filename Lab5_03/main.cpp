...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        {
            cv::aruco::drawAxis(image_raw, cameraMatrix,distCoeffs,rvecs[0],tvecs[0],10);
            stream1 << "rvecs: " << rvecs[0] ;
            stream2<< "tvecs: " << tvecs[0] ;
        }
        // Undistort
        cv::Mat image;
        ...
        // Display the image
            ...
            cv::putText(image, stream1.str(), cv::Point(10, 20), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 1, cv::LINE_AA);
            cv::putText(image, stream2.str(), cv::Point(30, 40), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 1, cv::LINE_AA);
